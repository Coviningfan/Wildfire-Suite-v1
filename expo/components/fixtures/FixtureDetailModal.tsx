          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            <View style={styles.hero}>
              <View style={styles.heroIcon}>
                <Lightbulb size={36} color={colors.primary} />
              </View>
              ...
              <View style={styles.heroBadges}>
                <Badge
                  icon={isDMX ? <Radio size={12} color={colors.accentLight} /> : <Zap size={12} color={colors.success} />}
                  label={controlType}
                  color={isDMX ? colors.accentLight : colors.success}
                />
                {powerWatts != null && (
                  <Badge icon={<Activity size={12} color={colors.secondary} />} label={`${powerWatts}W`} color={colors.secondary} />
                )}
              </View>
            </View>
            ...
            {data != null && (
              <View style={styles.section}>
                ...
                <SpecRow label="Beam Angle (H)" value={`${data.beam_h_deg}°`} colors={colors} styles={specSt} />
                <SpecRow label="Beam Angle (V)" value={`${data.beam_v_deg}°`} colors={colors} styles={specSt} />
                {data.field_h_deg != null && <SpecRow label="Field Angle (H)" value={`${data.field_h_deg}°`} colors={colors} styles={specSt} />}
                {data.field_v_deg != null && <SpecRow label="Field Angle (V)" value={`${data.field_v_deg}°`} colors={colors} styles={specSt} />}
                <SpecRow label="Peak Irradiance @ 1m" value={`${data.peak_irradiance_mWm2.toLocaleString()} mW/m²`} highlight colors={colors} styles={specSt} />
                {radiantPower != null ? (
                  <SpecRow
                  ...
                  <SpecRow label="Peak Power Density" value={`${(data.peak_irradiance_mWm2 / 1000).toFixed(3)} W/m²`} isLast colors={colors} styles={specSt} />
                )}
              </View>
            )}
            ...
            {powerWatts != null && (
              <View style={styles.section}>
                ...
                <SpecRow label="Consumption" value={`${powerWatts}W`} colors={colors} styles={specSt} />
                <SpecRow label="Current @ 120V" value={`${(powerWatts / 120).toFixed(2)}A`} colors={colors} styles={specSt} />
                <SpecRow label="Max on 20A (80% rule)" value={`${Math.floor(1920 / powerWatts)} units`} isLast colors={colors} styles={specSt} />
              </View>
            )}
              ...
              <View style={styles.docsGrid}>
                ...
                >
                  <View style={[styles.docIconWrap, { backgroundColor: 'rgba(124, 107, 240, 0.12)' }]}>
                    <FileDown size={16} color={colors.accent} />
                  </View>
                ...
                {specPageUrl && (
                  ...
                  >
                    <View style={[styles.docIconWrap, { backgroundColor: 'rgba(59, 130, 246, 0.12)' }]}>
                      <Calculator size={16} color={colors.focus} />
                    </View>
                ...
                >
                  <View style={[styles.docIconWrap, { backgroundColor: 'rgba(249, 115, 22, 0.12)' }]}>
                    <Shield size={16} color={colors.safetyOrange} />
                  </View>
                ...
                {comparisonUrl && (
                  ...
                  >
                    <View style={[styles.docIconWrap, { backgroundColor: 'rgba(59, 159, 232, 0.12)' }]}>
                      <BarChart3 size={16} color={colors.focus} />
                    </View>
                ...
                >
                  <View style={[styles.docIconWrap, { backgroundColor: 'rgba(155, 109, 255, 0.12)' }]}>
                    <Share2 size={16} color={colors.accentLight} />
                  </View>
...
function SpecRow({ label, value, highlight, isLast, colors, styles }: { label: string; value: string; highlight?: boolean; isLast?: boolean; colors: ThemeColors; styles: ReturnType<typeof createSpecStyles> }) {
  return (
    <View style={[styles.row, isLast === true && styles.rowLast]}>
      <Text style={styles.label}>{label}</Text>
      <Text style={[styles.value, highlight === true && { color: colors.primary }]}>{value}</Text>
    </View>
  );
}
...
function createSpecStyles(colors: ThemeColors) {
  return StyleSheet.create({
    ...
    value: { fontSize: 13, fontWeight: '600' as const, color: colors.text, textAlign: 'right' as const, flex: 1 },
  });
}
...
function createStyles(colors: ThemeColors) {
  return StyleSheet.create({
    ...
    heroModel: { fontSize: 24, fontWeight: '800' as const, color: colors.text, textAlign: 'center' as const, letterSpacing: -0.3 },
    heroCategory: { fontSize: 13, color: colors.textSecondary, marginTop: 4, textAlign: 'center' as const },
    heroSeries: { fontSize: 12, color: colors.textTertiary, marginTop: 2 },
    ...
    sectionTitle: { fontSize: 15, fontWeight: '700' as const, color: colors.text, marginBottom: 10, letterSpacing: -0.1 },
    dmxRow: {
    ...
    dmxCh: { fontSize: 11, fontWeight: '800' as const, color: colors.accentLight },
    dmxFn: { flex: 1, fontSize: 12, color: colors.text },
    dmxRange: { fontSize: 11, color: colors.textTertiary },
    infoBox: {
    ...
    docBtnLabel: { fontSize: 13, fontWeight: '600' as const, color: colors.text, marginBottom: 2 },
    docBtnSub: { fontSize: 10, color: colors.textTertiary },
    ...
    btnOutlineText: { fontSize: 15, fontWeight: '600' as const, color: colors.text },
    btnPrimary: {
